...
import com.av.carb.model.*;
import com.av.carb.repository.LotteryRepository;
import com.av.carb.response.CarACheckResponse;
    ...
    public Lottery saveLottery(String personNationalCode , String carBuildNumber) {
        ...
        try {
            restTemplate.getForEntity(
                    "http://localhost:8080/cara/lottery/{nationalCode}",
                    CarACheckResponse.class ,
                    personNationalCode
            );
            log.info("This person registered to lottery service A !!!");
            return new LotteryNullClass();
        }catch (HttpClientErrorException e){
            log.info("This person not registered before to lottery service A !!!");
